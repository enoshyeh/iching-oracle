...
/** Regenerate AI interpretation for a pending reading (owner only). */
export async function regenerateInterpretationForReading(
  ...
  if (!reading.interpretationPending) {
    return { success: false, error: "This reading already has an interpretation." };
  }
